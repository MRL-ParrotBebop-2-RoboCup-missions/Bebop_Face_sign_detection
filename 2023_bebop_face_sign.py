...
class face_sign_detection():
    def __init__(self) -> None:
        ...
        self.model=YOLO(self.path+'src/open2023/scripts/bebop_face_sign/best.pt','v8')
        self.bridge = CvBridge()
        
        rospy.init_node('bebop_face_sign', anonymous=True)
        rospy.Subscriber('/bebop/image_raw', Image, self.face_sign_callback)

        myfile = open(self.path+'/src/open2023/scripts/bebop_face_sign/classes1.txt','r')
        data = myfile.read()
        ...
        if len (DP) != 0 :
            for i in range(len(detect_params[0])):                   
                ...
                cv.putText(
                    ...
                    (255, 255, 255),
                    2)
                cv.imwrite(self.path+'src/open2023/scripts/bebop_face_sign/detected/'+ self.class_list[int(clsID)] + str(rnd)+'.jpg', cv_img)
